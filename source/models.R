...
get_bootstrap_estimates <- function(sim_data, beta_hat, true_beta, B=10, B_inner=5, alpha=0.05) {
	...
	boot_out <- foreach(b=1:B, .combine=rbind) %dorng% {
		...
		sim_data_b <- get_boot_sample(sim_data)
		model_lm <- fit_model(sim_data_b)
		boot_beta <- coef(model_lm)[['x']]

		# ## INNER LOOP - Bootstrap t
		boott_time <- func_time(t_star <- bootstrap_t_inner(sim_data_b, boot_beta, beta_hat, B_inner))

		out_b <- data.frame(boot_beta, boott_time, t_star)
	}

	# dataframe to vectors
	boot_beta <- boot_out[['boot_beta']]
	boott_time <- boot_out[['boott_time']]
	t_star <-  boot_out[['t_star']]
...
# bootstrap_out <- foreach(b=1:B, .combine=) %dorng% {
# 	set.seed(seeds[b])
# 	sim_data_b <- get_boot_sample(sim_data)
# 	model_lm <- fit_model(sim_data_b)
# 	boot_beta <- coef(model_lm)[['x']]
